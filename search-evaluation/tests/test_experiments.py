...
class ExperimentSelectionTest(unittest.TestCase):
    def test_preserves_recall_then_prefers_ranking_quality(self) -> None:
        ...
        improved = self.create_outcome("improved", 0.90, 0.80, 0.80, 0.40)

        chosen = choose_quality_preserving(baseline, [recall_regression, improved])

        self.assertEqual(chosen.config.name, "improved")
